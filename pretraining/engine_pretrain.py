...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        images = samples['img']
        input_image_path = samples['input_image_path'][0]
        ...
        with torch.cuda.amp.autocast():
            if args.model_type == "satmaepp":
                images_up_2x = samples['img_up_2x']
                images_up_4x = samples['img_up_4x']
                imgaes_up = [images_up_2x, images_up_4x]

                loss, _, _ = model(images, imgaes_up, mask_ratio=args.mask_ratio)
            else:
