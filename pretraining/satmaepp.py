...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,    proj_ratio=4):
        super().__init__()
    ...
    def patchify(self, imgs, p, c):
        ...
        x = x.reshape(shape=(imgs.shape[0], h * w, p ** 2 * c)) # N, L, cpp: L is number of patches
        return x
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # print("x after pos_embed: ", x.shape)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x = self.decoder_pred(x) # (N, L, c*p*p)
    ...
    def forward(self, imgs, imgs_up, mask_ratio=0.75):
        ...
        # print("-------------------------")
        orig_loss = self.forward_loss(imgs_wo_dem, pred, mask)

        ms_loss_2x = torch.nn.functional.l1_loss(pred_2x, imgs_up[0][:,:-1,:,:])
        ms_loss_4x = torch.nn.functional.l1_loss(pred_4x, imgs_up[1][:,:-1,:,:])

        ms_loss = ms_loss_2x*0.5 + ms_loss_4x*0.5 # multi-scale loss

        loss = 0.6*orig_loss + 0.4*ms_loss
