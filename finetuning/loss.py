...
#         print("gt_labels: ", gt_labels.shape)
        ...
        pred_label_idx = torch.unsqueeze(pred_label_idx, dim = 1)
...
#         print("flood_pred: ", flood_pred.shape)
        ...
        ## Generate Pred Masks
        ones = torch.ones_like(gt_labels).long()
#         h, w = gt_labels.shape[2], gt_labels.shape[3]
#         ones = torch.ones((1, 1, h, w))
#         print("ones: ", ones.shape)
        
        
#         flood_mask = torch.where(pred_label_idx == 0, ones, ones.new_tensor(0.0))
#         dry_mask = torch.where(pred_label_idx == 1, ones, ones.new_tensor(0.0))
        ...
        ones = torch.ones_like(gt_unfolded).long()
        zeros = torch.zeros_like(gt_unfolded).long()
