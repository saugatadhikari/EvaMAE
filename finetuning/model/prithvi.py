...
class PatchEmbedPrithvi(nn.Module):
    """ Frames of 2D Images to Patch Embedding
    The 3D version of timm.model.vision_transformer.PatchEmbedSpectralGPT
    """
...
class Prithvi(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16,
        ...
        self.patch_embed = PatchEmbedPrithvi(img_size, patch_size, num_frames, tubelet_size, in_chans, embed_dim)
        ...
        # self.decoder_pred = nn.Linear(decoder_embed_dim, tubelet_size * patch_size * patch_size * in_chans,
...
class PrithviSeg(nn.Module):
    def __init__(self, num_classes=1, embed_dim=768, encoder=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert encoder is not None
        self.encoder = encoder
        self.segmentation_head = SegmentationHead(embed_dim, num_classes)

    def forward(self, x):
        x, _, _ = self.encoder.forward_encoder(x, 0)
        x = x[:, 1:, :]
        B, N, C = x.shape
        x = x.transpose(1, 2).view(B, C, int(N ** 0.5), int(N ** 0.5))  # reshape to (B, C, H, W)
